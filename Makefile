TARGZ_FILE := amazon-cloudwatch-logs-for-fluent-bit.tar.gz
IMAGE_NAME := amazon-cloudwatch-logs-for-fluent-bit-package
...
%.build: Dockerfile.% rpmbuild/SPECS/amazon-cloudwatch-logs-for-fluent-bit.spec
	[ -d $@.bak ] && rm -rf $@.bak || :
	[ -d $@ ] && mv $@ $@.bak || :
